...
exports.up = function (knex, Promise) {
  return Promise.all([
    knex.schema.createTable('resource_plans', (table) => {
    ...
    knex.schema.table('projects', (table) => {
      table.integer('res_id').unsigned().references('res_id').inTable('resource_plans')
        .onDelete('CASCADE')
        .index();
    }),
    knex.schema.createTable('resource_allocations', (table) => {
      table.integer('res_id').unsigned().notNullable().references('res_id')
        .inTable('resource_plans')
        .onDelete('CASCADE')
        .index();
