...
exports.up = function(knex, Promise) {
  return Promise.all([
    ...
    knex.schema.createTable('category_data', function(table) {
      table.integer('cid').unsigned().notNullable().references('cid').inTable('categories').onDelete('CASCADE').index();
      table.integer('tid').unsigned().notNullable().references('tid').inTable('tasks').onDelete('CASCADE').index();
      table.integer('pid').unsigned().notNullable().references('pid').inTable('projects').onDelete('CASCADE').index();
    }),

    knex.schema.createTable('category_costs', function(table) {
      table.integer('cid').unsigned().notNullable().references('cid').inTable('categories').onDelete('CASCADE').index();
      table.integer('pid').unsigned().notNullable().references('pid').inTable('projects').onDelete('CASCADE').index();
